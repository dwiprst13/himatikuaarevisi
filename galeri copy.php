            ...
            ?>
                <div class="w-full sm:w-1/2 md:w-1/3 xl:w-1/4 p-2">
                    <button class="relative p-1 overflow-hidden group w-full" onclick="openModal('<?= $gambar ?>', '<?= $galeri['judul'] ?>', '<?= $galeri['deskripsi'] ?>')">
                        <figure class="bg-black w-full flex justify-center items-center overflow-hidden">
                            <img src="<?= $gambar ?>" alt="" class="object-cover aspect-w-6 aspect-h-4 h-48">
                        </figure>
                        <figcaption class="absolute p-1 top-0 left-0 w-full h-full">
                            <text class="w-full h-full opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-black bg-opacity-70 text-white flex flex-col items-center justify-center">
